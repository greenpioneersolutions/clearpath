...
- **Two auth checks must agree**: the launchpad readiness dot reads `AuthManager.checkCopilot`, but the session-start gate reads `CopilotAdapter.isAuthenticated` directly — if they disagree the user sees a green dot yet gets *"GitHub Copilot isn't signed in"* on New Chat. `~/.copilot/config.json` is **JSONC** (Copilot CLI prepends `//` banner comments), so both must parse it with the comment-tolerant `parseJsonc` shared util ([src/main/utils/jsonc.ts](src/main/utils/jsonc.ts)) — a plain `JSON.parse` throws on the banner and silently reports unauthenticated. `ClaudeCodeAdapter.isAuthenticated` mirrors `checkClaude` (incl. the macOS Keychain probe) for the same reason
...
- **stderr classification** ([outputClassification.ts](src/main/cli/outputClassification.ts), used by `CLIManager.attachListeners`) — `isUsageSummary` recognises Copilot's end-of-turn summary ("Changes +0 -0", "Requests N Premium", "Tokens ↑…↓… cached/reasoning") so it renders as a usage badge, **not** a red error block (regression: the old `premium request` regex missed the "N Premium" / arrow-token layout). Patterns are anchored to the summary shape so genuine errors mentioning "tokens"/"changes" aren't swallowed
...
### Slice 29: Session File Attachments — added 1.15.0 (flag `showFileAttachments`, default OFF)
- **The "real files" model, NOT content-injection.** Activates the previously-disabled "Files (soon)" launchpad chip. The user picks files; we **copy them into the workspace** at `.clear-path/uploads/<sessionId>/` (already gitignored) and hand them to the CLI **by path** — the agent reads/edits/runs them with its own file tools. Contrast with Notes, which inline file *text* into the prompt. Because the files live inside the session's `cwd`, **both Copilot and Claude reach them with zero adapter changes** (this deliberately sidesteps the gap that `CopilotAdapter.buildArgs` ignores `additionalDirs`/`--add-dir` entirely; Claude's emits it)
- **Reference-by-path framing** — `files:get-bundle-for-prompt` / exported `buildFilesBundle` emit a `<files count="N"><file name=… path=".clear-path/uploads/<sid>/…" size=… type=…/></files>` block, **paths only, never bytes**, so binaries + large files are safe and token cost is ~nil. Deterministic id-sort for prompt-cache stability; XML-escaped; empty selection → `framedPrompt: ''`. New `promptSlices.filesFramed` slice
- **Service** ([src/main/ipc/fileAttachmentHandlers.ts](src/main/ipc/fileAttachmentHandlers.ts)): manifest is `manifest.json` *inside* each upload dir (self-describing, no electron-store index). Standalone exported core — `getUploadsDir`, `stagePaths` (copy + validate + sha256-dedupe + collision `(2)` suffix + per-file 25MB / per-session 200MB caps from `FILE_ATTACHMENT_LIMITS`, overridable for tests), `buildFilesBundle`, `cleanupSessionUploads`, `sweepOrphanUploads`. IPC: `files:pick` (dialog only, no copy — for the launchpad before a session exists), `files:stage-paths` (copy at launch), `files:pick-and-stage` (mid-session convenience), `files:list`, `files:remove`, `files:get-bundle-for-prompt`, `files:open-folder`, `files:cleanup-session`, `files:sweep-orphans`. Security reuses [pathSecurity.ts](src/main/utils/pathSecurity.ts) (`assertPathWithinRoots` to the uploads dir, `isSensitiveSystemPath` on sources); atomic tmp+rename manifest writes
- **Staging sequence** — the launchpad pre-generates the session id (renderer `crypto.randomUUID()`) so files copy into `.clear-path/uploads/<sessionId>/` *before* the session starts; `cli:start-session` reuses the existing `SessionOptions.sessionId` (the `--session-id UUID` field) so `CLIManager.startSession` adopts that id (collision-guarded). Picking copies into the **resolved workspace dir** (Work.tsx `resolveWorkingDirectory`, not QuickStartCard's `app:get-cwd`) to avoid cwd mismatch
- **Frozen metadata + audit chip** — `attachedFiles?: {id,name,relPath}[]` threaded through `SessionOptions`, `OutputMessage`, `MessageLogEntry` ([CLIManager.ts](src/main/cli/CLIManager.ts)), persisted `messageLog` ([cli/types.ts](src/main/cli/types.ts)), and `cli:send-input`. Frozen at attach time; the gray in-chat chip ([OutputDisplay.tsx](src/renderer/src/components/OutputDisplay.tsx) `AttachedListChip kind="file"`, gated on `showFileAttachments`) shows **names only — file path/content never reach the DOM** (we inject paths, the chip shows names). `PersistedSession.workingDirectory` now persisted so the delete hook + sweep can locate uploads
- **Lifecycle** — `cli:delete-session(s)` ([handlers.ts](src/main/ipc/handlers.ts)) calls `cleanupSessionUploads`; Work.tsx fires `files:sweep-orphans` on mount to reclaim upload dirs with no live session (crash/manual-edit safety net)
- **Mid-session attach symmetry + workspace nudge** — `files:pick-and-stage` (compose "+", [Work.tsx](src/renderer/src/pages/Work.tsx) `handleAttachFiles`) and `files:stage-paths` both resolve a concrete dir via `ensureBaseDir` and return `{ baseDir, usedFallback }`. Previously the launchpad had the `ensureBaseDir` safety net but the mid-session path hard-failed with *"Select a workspace folder before attaching files."* whenever `resolveWorkingDirectory()` came back empty (no active workspace + no default cwd) — so attach worked at start but not mid-session. Now both paths fall back to the app-managed scratch dir and never hard-fail. `ActiveSessionState.workingDirectory` (set at start, refreshed from `baseDir` after each attach) is the SINGLE dir used for staging AND the matching `files:get-bundle-for-prompt`, so re-resolving can't split files across different uploads roots. `usedFallback` drives a non-blocking "Select a workspace →" nudge (`showWorkspaceNudge`, sky banner above `ChatInputArea`, routes to `/workspaces`) so files ideally land in the user's real repo next time. `PickAndStageResult` gained optional `baseDir` / `usedFallback` in [shared/files/types.ts](src/shared/files/types.ts)
- **UI** ([QuickStartCard.tsx](src/renderer/src/components/work/QuickStartCard.tsx)): flag-on renders an active "+ Files" chip + popover (file list with size + remove, "+ Add files" → `files:pick`); flag-off keeps the disabled "Files (soon)" placeholder. Copy in [launchpad.ts](src/renderer/src/copy/launchpad.ts)
- **Tests**: [fileAttachmentHandlers.test.ts](src/main/ipc/fileAttachmentHandlers.test.ts) (copy/dedupe/collision/caps/budget, reference-only framing + ordering + escaping, cleanup, orphan sweep — real temp-dir fs, electron mocked; plus `files:pick-and-stage` / `files:stage-paths` handler tests via a fake `ipcMain` asserting the no-workspace scratch fallback + `usedFallback`/`baseDir`, and `ensureBaseDir` fallback). OutputDisplay "no file path/content in DOM" guarantee. **Flag-on chip render covered manually** — the `setup-coverage.ts` eager-load defeats `vi.mock` of `useFlag` (same caveat as the Notes chip). E2e: [file-attachments.pw.spec.ts](e2e/file-attachments.pw.spec.ts) drives the real preload→handler round-trip for both the with-workspace and no-workspace-fallback cases (dialog-free `files:stage-paths`)
- **Deferred (future)**: hybrid inline of small text files, Copilot `--add-dir` support (only needed if uploads ever move outside `cwd`), drag-and-drop, image previews, a Settings budget control
