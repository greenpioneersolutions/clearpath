...
- **Tests**: [fileAttachmentHandlers.test.ts](src/main/ipc/fileAttachmentHandlers.test.ts) (copy/dedupe/collision/caps/budget, reference-only framing + ordering + escaping, cleanup, orphan sweep — real temp-dir fs, electron mocked). OutputDisplay "no file path/content in DOM" guarantee. **Flag-on chip render covered manually** — the `setup-coverage.ts` eager-load defeats `vi.mock` of `useFlag` (same caveat as the Notes chip)
- **Deferred (future)**: mid-session attach UI in the compose toolbar (the `files:pick-and-stage` IPC + `attachedFiles` plumbing already support it), hybrid inline of small text files, Copilot `--add-dir` support (only needed if uploads ever move outside `cwd`), drag-and-drop, image previews, a Settings budget control
