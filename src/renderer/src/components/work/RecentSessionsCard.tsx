...
import { timeAgo, messageCountLabel } from '../../lib/relativeTime'
          ...
          {recent.map((s) => (
            <li key={s.sessionId}>
              <button
                ...
                className="w-full h-full text-left px-3 py-2.5 rounded-lg border border-gray-800 hover:border-indigo-500/60 hover:bg-gray-800/40 transition-colors group space-y-1"
              >
                ...
                <div className="text-[11px] text-gray-500">{timeAgo(s.endedAt ?? s.startedAt)}</div>
                <div className="text-[11px] text-gray-500">{messageCountLabel(s.messageLog.length)}</div>
              </button>
            </li>
          ))}
        </ul>
