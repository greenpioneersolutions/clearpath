...
import McpCatalogGrid from '../components/mcp/McpCatalogGrid'
import McpRegistryList from '../components/mcp/McpRegistryList'
import type { McpSecretsMeta } from '../types/mcp'
import { useMcpExternalChanges } from '../hooks/useMcpExternalChanges'

// ── Tabs ──────────────────────────────────────────────────────────────────────

type Tab = 'catalog' | 'installed' | 'advanced'
...
export default function Connections(): JSX.Element {
  ...
  const handleCatalogInstallSaved = () => {
    // Bounce user to Installed and refresh the list.
    ...
    <div className="p-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">
          Add MCP servers that both CoPilot and Claude Code can use.
        </p>
      </div>

      {/* Tabs */}
      <div className="border-b border-gray-200">
        <nav className="flex gap-6" role="tablist" aria-label="Connections sections">
          {TABS.map((t) => (
            <button
              key={t.key}
